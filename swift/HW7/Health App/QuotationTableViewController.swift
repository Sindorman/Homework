...
class QuotationTableViewController: UITableViewController, UITextFieldDelegate {
    
    var quotes = [Quotation]()
    var index = 0
    var quoteToAdd: Quotation?
    var quoteOfTheDayID: String = ""
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (quote.id == self.quoteOfTheDayID)
        {
            cell.QuoteLabelText.textColor = UIColor.red
        }
        else
        {
            cell.QuoteLabelText.textColor = UIColor.black
        }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            // Setup alert
            let alert = UIAlertController(title: "Delete Quotation?", message: self.quotes[indexPath.row].quote, preferredStyle: .alert)
