...
class TableViewController: UITableViewController, UITextFieldDelegate {
    ...
    func initSettings()
    {
        if (UserDefaults.standard.object(forKey: "filterAnon") == nil) {
            UserDefaults.standard.set(false, forKey: "filterAnon")
        }
        if (UserDefaults.standard.object(forKey: "showAuthors") == nil) {
            UserDefaults.standard.set(true, forKey: "showAuthors")
        }
        if (UserDefaults.standard.object(forKey: "sortQuotes") == nil) {
            UserDefaults.standard.set(false, forKey: "sortQuotes")
        }
    }
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if (UserDefaults.standard.bool(forKey: "filterAnon"))
        {
            var count = 0
    ...
    @IBAction func unwindFromQuoteView (sender: UIStoryboardSegue) {
        ...
        else if (sender.identifier == "unwindFromSettingsView")
        {
            self.tableView.reloadData()
        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var quote: Quotation?
        let sortedQuotes = self.quotes.sorted(by: {$0.quote < $1.quote})
        if(UserDefaults.standard.bool(forKey: "sortQuotes"))
        {
            quote = sortedQuotes[indexPath.row]
        }
        else
        {
            quote = self.quotes[indexPath.row]
        }
        
        if (UserDefaults.standard.bool(forKey: "filterAnon"))
        {
            var index = indexPath.row
            while (quote?.author == nil)
            {
                if(UserDefaults.standard.bool(forKey: "sortQuotes"))
                {
        ...
        if (UserDefaults.standard.bool(forKey: "showAuthors"))
        {
            cell.AuthorLabelText?.text = quote!.author == nil ? "anonymous" : quote!.author
        }
        else
        {
            cell.AuthorLabelText?.text = ""
        }
        ...
        if editingStyle == .delete {
            ...
            if (UserDefaults.standard.bool(forKey: "filterAnon"))
            {
                for quote in self.quotes
                {
                    ...
                    clearQuotes.append(quote)
                }
            }

            if(UserDefaults.standard.bool(forKey: "sortQuotes"))
            {
                let sortedQuotes = clearQuotes.sorted(by: {$0.quote > $1.quote})
                let quote = sortedQuotes[indexPath.row]
                let deleteIndex = self.quotes.firstIndex(where: {$0.quote == quote.quote})
                self.quotes.remove(at: deleteIndex!)
            }
            else
            {
