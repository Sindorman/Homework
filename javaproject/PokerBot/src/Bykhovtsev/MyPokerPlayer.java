...
public class MyPokerPlayer implements PokerPlayer {
	...
	public PokerDecision decide(PokerGameDetails game, int betRequiredToCall) {
		...
		if (this.RR < 1.0) {
			...
			checked = false;
			return new PokerDecision(PokerDecision.TYPE.CALL);
		}else if (this.RR >= 1.3 && game.minRaise() + betRequiredToCall + 3 < Chips) {
			this.RR = 0;
			checked = false;
	...
	public double ReallyHelp(int NumberOfPlayers, PokerCard one, PokerCard two, ArrayList<PokerCard> community) {
