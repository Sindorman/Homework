...
public class HashIntSet {
	 ...
	 public boolean containsAll(HashIntSet set) {
		 ...
		 String[] aux = set.toString().split(" ");
		 int[] temp = new int[aux.length];
		 for (int x = 0; x < aux.length - 1; x ++) {			
			 if (x == 0) {
				 temp[x] = Integer.parseInt(aux[x].substring(1, aux[x].length() - 1));
			 }else {
				 temp[x] = Integer.parseInt(aux[x].substring(0, aux[x].length() - 1));
			 }
		 }
		 boolean [] ret = new boolean[temp.length];
		 for (int i = 0; i < temp.length - 1; i++) {			 
				 ret[i] = contains(temp[i]);			 
		 }
		 for (int x = 0; x < ret.length - 1; x++) {
			 if (ret[x] == false) {
				 return false;
			 }
		 }
	 ...
	 public int[] toArray() {
		 if (size() == 0) {
			...
			int[] temp = new int[1];
			String aux = toString();
			temp[0] = Integer.parseInt(aux.substring(1, aux.length() - 1));
			return temp;
		 }
